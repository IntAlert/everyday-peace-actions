...
const botly = new Botly({
    verifyToken: 'lkasdnflksafnlksadfnlaksdfnlkasdfnlkasdf',
    accessToken: process.env.ACCESS_TOKEN
});

var tasks = require('./tasks/index.js')(botly)
...
botly.on('message', (sender, message, data) => {
    ...
    if (users[sender]) {
        ...
        else if (data && data.text && data.text.indexOf('challenge') !== -1) {
            var element = tasks.cake.challenge
        ...
        botly.getUserProfile(sender, function (err, info) {
            ...
            botly.sendText({id: sender, text: `Hello ${users[sender].first_name}! Write 'challenge' if you want a challenge..`}, function (err, data) {
...
botly.on('postback', (sender, message, postback) => {
    ...
    if (postback == 'more') {
        var text = tasks.cake.more.text
        botly.sendText({id: sender, text:text, quick_replies: [botly.createQuickReply('Accept Challenge', 'accept')]}, function (err, data) {
            console.log('send generic cb:', err, data);
        });

    }

    else if (postback == 'accept') {
        var text = "Great! Here is some more info"
        botly.sendText({id: sender, text:text}, function (err, data) {
            
            var element = tasks.cake.accept
            botly.sendGeneric({id: sender, elements:element, aspectRatio: Botly.CONST.IMAGE_ASPECT_RATIO.SQUARE}, function (err, data) {
                
                var text = "We will be in touch in a few days to see how you did? Good luck! If you finish before we prompt you, click did it below!"
                botly.sendText({id: sender, text:text, quick_replies: [botly.createQuickReply('Did it!', 'complete')]}, function (err, data) {
                    console.log('send generic cb:', err, data);
                });
    ...
    else if (postback == 'complete') {
        var text = "You really are the best. Well done, would you like to Share your acheivement on your Facebook feed?"
        botly.sendText({id: sender, text:text}, function (err, data) {
            
            var element = tasks.cake.share
            botly.sendGeneric({id: sender, elements:element, aspectRatio: Botly.CONST.IMAGE_ASPECT_RATIO.SQUARE}, function (err, data) {
